...
public class PrinterTest {
	...
	public static void main(String[] args) throws IOException, PrintException {
		BufferedImage image = ImageIO.read(new File("./images/jhlabs.image.png"));
		print(image);
	}

	public static void print(BufferedImage image) throws PrintException, IOException {
		// BufferedImage jhlabs.image = ImageIO.read(new File("./images/jhlabs.image.png"));
	...
	public static void print(BufferedImage image, int h) {
		...
		for (int j = h; j < h + 8; j++) {
			for (int i = 0; i < width; i += 8) {
				...
				for (int k = 0; k < 8; k++) {
					...
					// if (i + 8 + k < jhlabs.image.getHeight())
					// b += jhlabs.image.getRGB(j, i + k + 8) == -1 ? 0 : 1;
					// if (i + 16 + k < jhlabs.image.getHeight())
					// c += jhlabs.image.getRGB(j, i + k + 16) == -1 ? 0 : 1;
					// }
				}
				data[cnt++] = a;
			}
		...
		// // b <<= 1;
		// // c <<= 1;
		// if (i + k < jhlabs.image.getHeight())
		// a += jhlabs.image.getRGB(j, i + k) == -1 ? 0 : 1;
		// // if (i + 8 + k < jhlabs.image.getHeight())
		// // b += jhlabs.image.getRGB(j, i + k + 8) == -1 ? 0 : 1;
		// // if (i + 16 + k < jhlabs.image.getHeight())
		// // c += jhlabs.image.getRGB(j, i + k + 16) == -1 ? 0 : 1;
