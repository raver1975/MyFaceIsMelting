...
public class PhotoBoothAndCaricature implements KeyListener, Printable {
	...
	public void start() throws Exception {
		...
		IplImage image = new OpenCVFrameConverter.ToIplImage().convertToIplImage(frame);
		...
		while (running) {
			...
			image = new OpenCVFrameConverter.ToIplImage().convertToIplImage(frame);
//			jhlabs.image = grabber.grab();

			CvMemStorage storage = CvMemStorage.create();
			opencv_core.CvSeq sign = cvHaarDetectObjects(image, face_cascade, storage, 1.5, 3, CV_HAAR_DO_CANNY_PRUNING);
			...
			if (total_Faces > 0) {
				...
				if (mustacheOn && mustdetect) {
					BufferedImage combined = new Java2DFrameConverter().getBufferedImage(new OpenCVFrameConverter.ToIplImage().convert(image));//new OpenCVFrameConverter.ToIplImage().convert();
					Graphics g = combined.getGraphics();
					g.drawImage(resizeImage(mustache, x1, y1), x2, y2, null);
					image.release();
					image=new OpenCVFrameConverter.ToIplImage().convert(new Java2DFrameConverter().getFrame(combined));
//					jhlabs.image = IplImage.createFrom(combined);
					mustdetect = false;
				}
			...
			cf1.showImage(new OpenCVFrameConverter.ToIplImage().convert(temp12));
			// -----------------------------------------------------------------------------------------------------
			// cf1.showImage(copy);
			...
			BufferedImage bfi1=new Java2DFrameConverter().convert(new OpenCVFrameConverter.ToIplImage().convert(copy));
			BufferedImage bfi2=AutoCorrectionFilter.filter(bfi1);
			BufferedImage bfi3=resizeImage(AutoCorrectionFilter.filter(bfi2), maxwidth, newheight);
			IplImage copy1=new OpenCVFrameConverter.ToIplImage().convert(new Java2DFrameConverter().convert(bfi2));
			copy=new OpenCVFrameConverter.ToIplImage().convert(new Java2DFrameConverter().convert(bfi3));			IplImage gray3 = IplImage.create(copy.cvSize(), IPL_DEPTH_8U, 1);
			...
			saveImage = new Java2DFrameConverter().convert(new OpenCVFrameConverter.ToIplImage().convert(gray3));
			...
			cvResetImageROI(image);
			cf2.showImage(new OpenCVFrameConverter.ToIplImage().convert(image));
			_3image.release();
	...
	public static IplImage render(IplImage image, AbstractBufferedImageOp rf) {
		...
		image.release();
		image = new OpenCVFrameConverter.ToIplImage().convert(new Java2DFrameConverter().convert(bi1));
		return image;
	}
	...
	public static BufferedImage toBufferedImage(Image img) {
		...
		// Create a buffered jhlabs.image with transparency
		BufferedImage bimage = new BufferedImage(img.getWidth(null), img.getHeight(null), BufferedImage.TYPE_INT_ARGB);

		// Draw the jhlabs.image on to the buffered jhlabs.image
		Graphics2D bGr = bimage.createGraphics();
		bGr.drawImage(img, 0, 0, null);
		bGr.dispose();

		// Return the buffered jhlabs.image
		return bimage;
	}
