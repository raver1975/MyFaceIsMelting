...
private final static int greyRange = 256;

static public BufferedImage HistogramEqualization(BufferedImage image) throws IOException {
    ...
    BufferedImage originalImage = getOriginalImage(image, imageLUT);
    return originalImage;
}

public static void histogramEqualization(float histogram[], int low, int high) {
...
public static BufferedImage getOriginalImage(BufferedImage original, ArrayList<int[]> histLUT) {
    ...
    for (int i = 0; i < original.getWidth(); i++) {
        for (int j = 0; j < original.getHeight(); j++) {
            ...
            // Write pixels into jhlabs.image
...
public static void normalizedFunction(float myArr[], int low, int high) {
...
public static float[] getHistogramByColor(BufferedImage input, int colorVal) {
